...
public class AmplifyBouncy : MonoBehaviour
{
    ...
    public bool shouldUseClassicBounce = false;
    ...
    void OnCollisionEnter2D(Collision2D coll)
    {
        if (IsBounceable(coll.gameObject))
        {
            ...
            if (shouldUseClassicBounce)
            {
                coll.collider.GetComponent<Rigidbody2D>().AddForce(new Vector2(velocity.x, velocity.y) * 7f * 100);
            }
            else
            {
