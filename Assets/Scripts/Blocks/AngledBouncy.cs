...
public class AngledBouncy : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D coll)
    {
        if (coll.gameObject.CompareTag("Player"))
        {
            ...
            Vector2 bounceForceVector = new Vector2((pControl.playerAnimation.isFacingRight ? 500 : -500), 500) * bounceForce;
            Debug.Log("Bounce force vector: " + bounceForceVector);
            pControl.transform.GetComponent<Rigidbody2D>().velocity = Vector2.zero;
            Debug.Log("Bounce rigidBody: " + pControl.transform.GetComponent<Rigidbody2D>().velocity);
            pControl.transform.GetComponent<Rigidbody2D>().AddForce(bounceForceVector);
